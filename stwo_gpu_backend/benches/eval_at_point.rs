...
use stwo_gpu_backend::examples::get_env_var;

fn bench_eval_at_secure_point<B: PolyOps>(c: &mut Criterion, id: &str) {

    let min_log = get_env_var("MIN_LOG", 7u32);
    let max_log = get_env_var("MAX_LOG", 28u32);

    for log_size in min_log..=max_log {
...
fn eval_at_secure_point_benches(c: &mut Criterion) {
    ...
    bench_eval_at_secure_point::<CudaBackend>(c, "stwo-gpu");
}
